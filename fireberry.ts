...
class Fireberry {
    ...
    //Utils
    private async sendRequest(url, method, data: any = {}): Promise<{ data?: any; error?: boolean }> {
        try {
            const headers = {
    ...
    private async getObjectData(objecttype: Type, query = "", page_number = 1, page_size = 500, sort_type = "desc", sort_by = "createdon", accumulator: any[] = []) {
        if (!this.validateInputs({ objecttype })) {
            return null;
        }
